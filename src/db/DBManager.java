...
/**
 * Database Manager used to open a connection to a database,
 * perform create, read, update, and delete procedures, and 
...
public class DBManager {
	...
	/**
	 * 0-param constructor instantiating the ContactTableModel,
	 * MailTableModel, FolderTbaleModel, MailProperties, and
	 * MailConfigBean.
	 */
	public DBManager(){
	...
	/**
	 * Connection method used to access the database specified in the MailProperties.
	 */
	private void connect(){
		try{
			//Creating the connection url consisting of protocols, server, port number, and database name. 
			String url = "jdbc:mysql://"+configs.getDBServer()+":"+configs.getDBPort()+"/"+configs.getDBName();
			String user = configs.getDBLogin();
			String password = configs.getDBPassword();
			//Creating a connection to the database using the connetion url, user name, and password
			...
			if(!tables.next()){
				...
				statement.execute("create table mail_table("+
			...
			if(!tables.next()){
				statement = connection.createStatement();
				statement.execute("create table folders_table("+
								"id		int unsigned not null primary key auto_increment,"+
								"folder	varchar(15) not null"+
								")");
				statement.execute("insert into folders_table values(null, 'Inbox')");
				statement.execute("insert into folders_table values(null, 'Sent')");
				statement.execute("insert into folders_table values(null, 'Draft')");
				statement.execute("insert into folders_table values(null, 'Deleted')");
			}
	...
	private void close(){
		try{
			...
			JOptionPane.showMessageDialog(null, "Unable to close connection to database!",
										  "JDBC Exception", JOptionPane.ERROR_MESSAGE);
	...
	//* MAIL ACCESS METHODS *\\
	/**
	 * Opens a connection the database, retrieves all mail messages,then closes the connection.
	...
	public List<MailBean> getAllMail(){
		...
		try{
			...
			while(results.next()){
				mail.add(new MailBean(results.getString(1),results.getString(2),
			...
			JOptionPane.showMessageDialog(null, "Unnable to retrieve all mail!",
										  "JDBC Exception", JOptionPane.ERROR_MESSAGE);
	...
	public boolean updateMail(MailBean mail){
		...
		try{
			...
			//if more than one row is affected, something failed, setting valid to false
			if(records !=1)
				valid=false;
	...
	/**
	 * Updated the specified MailBean's 'folder' value to Deleted
	 * in the databse and model.
	 * @param mail value to send to the Deleted folder
	 ...
	 * @param mail value to be inserted into the database.
	 * @return true if the MailBean was successfully inserted into the database, orelse false.
	 */
	private boolean insertMail(MailBean mail){
		...
		try{
			...
			statement = connection.prepareStatement("INSERT INTO mail_table "+
			...
			statement.setString(4, mail.getSubject()==null?"":mail.getSubject());
			statement.setString(5, mail.getCc()==null?"":mail.getCc());
			statement.setString(6, mail.getBcc()==null?"":mail.getBcc());
			statement.setString(7, mail.getDate()==null?"":mail.getDate());
			statement.setString(8, mail.getMessage()==null?"":mail.getMessage());
			statement.setString(9, mail.getFolder()==null?"":mail.getFolder());
			...
			row.next();
			mailModel.insertMail(new MailBean(""+row.getInt(1),mail.getTo(),mail.getFrom(),
											  mail.getSubject(),mail.getCc(),mail.getBcc(),
											  mail.getDate(),mail.getMessage(),mail.getFolder()));
		}
		catch(SQLException e){
	...
	public boolean deleteMail(){
		...
		if(beans.size() != 0){
			...
			try{
				for(int i = 0;i<ctr;i++){
					//Just realized this should probably be "Delete from mail_table where folder='Delete'"
					statement = connection.prepareStatement("DELETE FROM mail_table WHERE id= ?");
	 ...
	 * @param folder name to retrieve mail from.
	...
	public List<MailBean> getMailByFolder(String folder){
		...
		if(folder!=null && folder.length()>0){
			...
			try{
				...
				results=statement.executeQuery("SELECT * FROM mail_table WHERE folder = '"+folder+"'");
	 ...
	 * @param isDate indicates whether we are searching by date or by e-mail address
	 * @param dateOrAddress the data or address to search for.
	 * @return List containing all mail records retrieved.
	 */
	public List<ContactBean> getMailByAddressOrDate(boolean isDate, String dateOrAddress){
		List<ContactBean> contacts = new ArrayList<ContactBean>();
		PreparedStatement statement = null;
		ResultSet results = null;
		connect();
		try{
			//preparing the statement based on whether out search key is a date or e-mail.
			statement = connection.prepareStatement("SELECT * FROM mail_table WHERE "+(isDate?"received=":"sender=")+"?");
			...
			while(results.next()){
				contacts.add(new ContactBean(results.getString(1),results.getString(2),results.getString(3),
											  results.getString(4)));
			}
			...
			JOptionPane.showMessageDialog(null, "Unnable to retrieve contact by name or address!",
										  "JDBC Exception", JOptionPane.ERROR_MESSAGE);
			logger.log(Level.WARNING, "Problem retrieving records!",e);
			}
		finally{close();}
	...
	public List<ContactBean> getAllContacts(){
		...
		try{
			...
			ResultSetMetaData mtd = results.getMetaData();
			//load meta data into the contact table model to create the column names
			contactModel.loadColumnNames(mtd);
			
		}
		catch(SQLException e){
			JOptionPane.showMessageDialog(null, "Unnable to retrieve all contacts!",
										  "JDBC Exception", JOptionPane.ERROR_MESSAGE);
			logger.log(Level.WARNING, "Problem retrieving records!",e);
			}
		finally{close();}
	...
	private boolean insertContact(ContactBean contact){
		...
		try{
			...
			statement = connection.prepareStatement("INSERT INTO contacts_table "+
			...
			contactModel.insertContact(new ContactBean(""+row.getInt(1),contact.getfName(),
										               contact.getlName(),contact.getEmail()));
		}
		catch(SQLException e){
	...
	/**
	 * Saves a contact to the database, either by updating it, or inserting if it doesn't exist.
	 * @param contact value to save.
	 ...
	 * @param contact value to be removed from the database.
	...
	public boolean deleteContact(ContactBean contact){
		...
		if(contact != null & !contact.getId().equals("-1")){
			...
			try{
				...
				statement = connection.prepareStatement("DELETE FROM contacts_table WHERE id="+contact.getId());
				//execute the statement, storing the affected rows.
				int records = statement.executeUpdate();
				//if more or less than 1 row has been affected, something failed.
				if( records != 1)
					valid = false;
				//update changes to the contact table model.
	...
	public List<String> getAllFolders(){
		...
		try{
			...
			JOptionPane.showMessageDialog(null, "Unnable to retrieve all folders!",
										  "JDBC Exception", JOptionPane.ERROR_MESSAGE);
			logger.log(Level.WARNING, "Problem retrieving records!",e);
			}
		finally{close();}
	 ...
	 * @param name value of the folder to be inserted
    ...
    public boolean insertFolder(String name){
        ...
        try{
        		//preparing statement
                statement = connection.prepareStatement("INSERT INTO folders_table (id, folder) VALUES (?,?)");
     ...
     * 
     * @param name value of the folder to delete
    ...
    public boolean deleteFolder(String name){
        ...
        if(!name.equals("")){
            ...
            try{
            	//preparing the SQL statement.
                statement = connection.prepareStatement("SELECT * FROM folders_table WHERE folder='"+name+"'");
                //executing the statement, checking if the folder exists.
                boolean records = statement.execute();
                //if the folder exists, it is deleted.
                if(records){
                        statement = connection.prepareStatement("DELETE FROM folders_table WHERE folder='"+name+"'");
                ...
                JOptionPane.showMessageDialog(null, "You must enter a folder to delete!");
        return valid;
    }
